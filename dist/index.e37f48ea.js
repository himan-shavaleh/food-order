...
var _foodsView = require("./views/foodsView");
var _foodsViewDefault = parcelHelpers.interopDefault(_foodsView);
var _paginationView = require("./views/paginationView");
var _paginationViewDefault = parcelHelpers.interopDefault(_paginationView);
var _bodyView = require("./views/bodyView");
var _bodyViewDefault = parcelHelpers.interopDefault(_bodyView);
var _mealView = require("./views/mealView");
var _mealViewDefault = parcelHelpers.interopDefault(_mealView);
var _wishlistView = require("./views/wishlistView");
var _wishlistViewDefault = parcelHelpers.interopDefault(_wishlistView);
var _cartView = require("./views/cartView");
var _cartViewDefault = parcelHelpers.interopDefault(_cartView);
var _accountView = require("./views/accountView");
var _accountViewDefault = parcelHelpers.interopDefault(_accountView);
const controlCategoriesResult = async ()=>{
    await _model.loadCategories();
    (0, _categoriesViewDefault.default).render(_model.state.categories);
};
const controlMealsResult = async (goto = 1, query)=>{
    await _model.loadMainFoods(query);
    (0, _foodsViewDefault.default).render(_model.getMealsForPage(goto));
    await (0, _paginationViewDefault.default).render(_model.state);
};
const controlPagination = async (query)=>{
    await (0, _paginationViewDefault.default).render(_model.state);
};
const controlCategoryMeals = async (catName, goto = 1)=>{
    await _model.getCategoryMeals(catName);
    (0, _foodsViewDefault.default).render(_model.getMealsForPage(goto));
    (0, _paginationViewDefault.default).render(_model.state);
};
// search controlers
const searchControl = async (foodName)=>{
    await _model.getSearchFood(foodName);
    (0, _mealViewDefault.default).render(_model.state.meal);
};
//document controls
const setWishlistMeal = (meal)=>{
    if (_model.state.wishlist.find((mealx)=>meal.idMeal === mealx.idMeal)) _model.state.wishlist = _model.state.wishlist.filter((mealx)=>meal.idMeal == mealx.idMeal);
    else _model.state.wishlist.push(meal);
    return _model.state.wishlist;
};
const removeWishlistControler = (id)=>{
    _model.state.wishlist = _model.state.wishlist = _model.state.wishlist.filter((meal)=>meal.idMeal != id);
};
const getMealByIdControler = async (id)=>{
    await _model.getMealById(id);
    (0, _mealViewDefault.default).render(_model.state.meal);
};
...
const getCartItemsControler = ()=>{
    ...
    console.log(_model.state.cart);
};
const setCartItemControler = (item)=>{
    const itemInCartIndex = _model.state.cart.items.findIndex((meal)=>meal.id === item.id);
    if (itemInCartIndex >= 0) {
        _model.state.cart.items[itemInCartIndex].amount += item.amount;
        _model.state.cart.totalPrice += +item.amount * +item.price;
    } else if (itemInCartIndex === -1) {
        _model.state.cart.items.push(item);
        _model.state.cart.quantity++;
        _model.state.cart.totalPrice += +item.amount * +item.price;
    }
};
const removeCartItemControler = (id)=>{
    const currentItem = _model.state.cart.items.find((item)=>item.id == id);
    console.log(currentItem);
    _model.state.cart.totalPrice -= currentItem.amount * currentItem.price;
    _model.state.cart.items = _model.state.cart.items.filter((item)=>item.id != id);
    _model.state.cart.quantity--;
    (0, _cartViewDefault.default).render(_model.state.cart);
};
const accountControler = ()=>{
    (0, _accountViewDefault.default).render(_model.state.account);
};
const loginRengerControler = (acc)=>{
    _model.state.account.isLoggedIn = true;
    _model.state.account.loggedInUser = acc.id;
    controlMealsResult(1, "g");
};
const addUserControler = (user)=>{
    console.log(123);
    _model.state.account.users.push(user);
};
const init = ()=>{
    controlCategoriesResult();
    controlMealsResult();
    controlPagination();
    (0, _paginationViewDefault.default).addHandlerClick(controlCategoryMeals);
    (0, _paginationViewDefault.default).addHandlerClick(controlPagination);
    (0, _categoriesViewDefault.default).addHandlerClick(controlCategoryMeals);
    (0, _bodyViewDefault.default).addHandlerClick(searchControl);
    (0, _foodsViewDefault.default).addWishlistHandlerClick(setWishlistMeal);
    (0, _bodyViewDefault.default).showWishlistHandlerClick(getWishlistMealsControler);
    (0, _wishlistViewDefault.default).removeFromWishlistHandlerClick(removeWishlistControler, _model.state.wishlist);
    (0, _mealViewDefault.default).showMealHandlerClick(getMealByIdControler, _model.state.meal);
    (0, _bodyViewDefault.default).showOffersMealsHandlerClickc(controlMealsResult);
    (0, _bodyViewDefault.default).showCartHandlerClick(getCartItemsControler);
    (0, _foodsViewDefault.default).addToCartHandlerClick(setCartItemControler);
    (0, _cartViewDefault.default).removeFromCartHandlerClick(removeCartItemControler);
    (0, _mealViewDefault.default).addToCartHandlerClick(setCartItemControler);
    (0, _bodyViewDefault.default).openAccountHandlerClick(accountControler);
    (0, _accountViewDefault.default).loginBtnHandlerClick(loginRengerControler);
    (0, _accountViewDefault.default).gotoRegisterPageHandlerClick();
    (0, _accountViewDefault.default).signupBtnHandlerClick(loginRengerControler, addUserControler);
};
init();

},{"core-js/modules/es.array.includes.js":"dkJzX","core-js/modules/web.immediate.js":"49tUX","boxicons":"1afqO","./model":"Y4A21","./views/categoriesView":"4qGqK","regenerator-runtime/runtime":"dXNgZ","regenerator-runtime":"dXNgZ","./views/foodsView":"e1fk9","./views/paginationView":"6z7bi","./views/bodyView":"5iwqM","./views/mealView":"7kqiy","./views/wishlistView":"1eom8","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./views/cartView":"IaDwG","./views/accountView":"hPmPF"}],"dkJzX":[function(require,module,exports) {
"use strict";
...
},{"../internals/export":"dIGt4","../internals/global":"i8HOC","../internals/task":"7jDg7"}],"1afqO":[function(require,module,exports) {
...
parcelHelpers.export(exports, "loadMainFoods", ()=>loadMainFoods);
parcelHelpers.export(exports, "getMealsForPage", ()=>getMealsForPage);
parcelHelpers.export(exports, "getCategoryMeals", ()=>getCategoryMeals);
parcelHelpers.export(exports, "getSearchFood", ()=>getSearchFood);
parcelHelpers.export(exports, "getMealById", ()=>getMealById);
var _regeneratorRuntime = require("regenerator-runtime");
const state = {
    account: {
        isLoggedIn: false,
...
const getCategoryMeals = async (catName)=>{
    let res;
    if (catName === "") res = await fetch("https://www.themealdb.com/api/json/v1/1/search.php?f=b");
    else res = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${catName}`);
    const data = await res.json();
    state.meals = [];
    state.category = catName;
    state.meals = data.meals;
};
const getSearchFood = async (foodName)=>{
    ...
    addHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            ...
            if (e.target.closest(".category")) {
                const catName = e.target.closest(".category").querySelector(".category-name").textContent.toLowerCase();
                handler(catName);
            }
        });
    }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"e1fk9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class foodView extends (0, _viewDefault.default) {
    _parentElement = document.querySelector("#meals");
    addToCartHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".increase-amount")) e.target.closest(".add-form").querySelector("#amount").value++;
            if (e.target.closest(".decrease-amount")) {
                if (e.target.closest(".add-form").querySelector("#amount").value > 1) e.target.closest(".add-form").querySelector("#amount").value--;
            }
            if (e.target.closest(".submitAdd")) {
                if (e.target.closest(".meal")) {
                    const item = {
                        amount: +e.target.closest(".add-form").querySelector("#amount").value,
                        name: e.target.closest(".meal").querySelector(".meal-name").textContent,
                        img: e.target.closest(".meal").querySelector(".meal-image").getAttribute("src"),
                        price: +e.target.closest(".meal").querySelector(".price").textContent,
...
`);
    ...
    addWishlistHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".heart")) {
                const name = e.target.closest(".meal").querySelector(".meal-name").textContent;
                const id = e.target.closest(".meal").dataset.id;
                const img = e.target.closest(".meal").querySelector(".meal-image").getAttribute("src");
                const price = e.target.closest(".meal").querySelector(".price").textContent;
                const meal = {
                    idMeal: id,
                    strMealThumb: img,
                    price: price,
                ...
                e.target.closest(".heart").querySelector(".bx").classList.toggle("bxs-heart");
                e.target.closest(".heart").querySelector(".bx").classList.toggle("bx-heart");
                if (e.target.closest(".heart").querySelector(".bx").classList.contains("bx-heart")) --document.querySelector(".wishlist-counter").textContent;
                else if (e.target.closest(".heart").querySelector(".bx").classList.contains("bxs-heart")) ++document.querySelector(".wishlist-counter").textContent;
                // console.log(meal);
                handler(meal);
            }
        });
    ...
    <span class="heart"><i class='bx bx-heart' ></i></span>
      ...
      <div class="description">
        ...
        <p class="price-content">price: <span class="price">${(+food.idMeal / (Math.random() * 100000)).toFixed(2)}</span> $</p>
...
class paginationView extends (0, _viewDefault.default) {
    ...
    _generateMarkup() {
        this._parentElement.innerHTML = "";
        const pagesNumber = Math.ceil(this._data.meals.length / this._data.mealsPerPage);
...
},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5iwqM":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class bodyView extends (0, _viewDefault.default) {
    _parentElement = document.querySelector("body");
    openAccountHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".account")) handler();
        });
    }
    addHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            ...
            if (e.target.closest(".search")) document.querySelector(".modal-search").classList.toggle("hidden");
            if (e.target.closest(".close-modal")) e.target.closest(".modal").classList.add("hidden");
            if (e.target.closest(".overlay")) e.target.closest(".modal").classList.add("hidden");
            ...
            if (e.target.closest(".home")) window.location.reload();
        });
    }
    showOffersMealsHandlerClickc(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".offers")) handler(1, "c");
        });
    }
    showCartHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".show-cart-btn")) handler();
        });
    }
    showWishlistHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".wishlist")) {
                ...
                if (data.length === 0) // document.querySelector(".wishlist-meals").innerHTML = "";
                document.querySelector(".wishlist-meals").innerHTML += `<p>you have no wishlist item</p>`;
                if (data.length !== 0) data.forEach((meal)=>{
                    // document.querySelector(".wishlist-meals").innerHTML = "";
                    document.querySelector(".wishlist-meals").innerHTML += `
            ...
            <div class="description">
              ...
              <p>price: <span class="price">${(+meal.idMeal / (Math.random() * 100000)).toFixed(2)}</span></p>
            </div>
            <div class="controls">
            ...
            if (e.target.closest(".wishlist")) document.querySelector(".wishlist-modal").classList.remove("hidden");
            if (e.target.closest(".close-modal")) e.target.closest(".modal").classList.add("hidden");
            if (e.target.closest(".overlay")) e.target.closest(".modal").classList.add("hidden");
        });
    }
}
exports.default = new bodyView();

},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7kqiy":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class mealView extends (0, _viewDefault.default) {
    _parentElement = document.querySelector("#meals");
    addToCartHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".increase-amountm")) e.target.closest(".add-form").querySelector("#amount").value++;
            ...
            if (e.target.closest(".submitAdd")) {
                const item = {
                    amount: +e.target.closest(".add-form").querySelector("#amount").value,
                    name: e.target.closest(".meal-view").querySelector(".meal-name").textContent,
                    img: e.target.closest(".meal-view").querySelector(".meal-image").getAttribute("src"),
                    price: +e.target.closest(".meal-view").querySelector(".price").textContent,
        ...
        <div class="meal-view" data-id="${this._data.idMeal}">
          <div class="header">
            ...
            <div class="description">
              ...
              ${this._data.strIngredient1 && `<p>ing 1:${this._data?.strIngredient1}</p>`}
              ${this._data.strIngredient2 && `<p>ing 2:${this._data?.strIngredient2}</p>`}
              ${this._data.strIngredient3 && `<p>ing 3:${this._data?.strIngredient3}</p>`}
              ${this._data.strIngredient4 && `<p>ing 4:${this._data?.strIngredient4}</p>`}
              ${this._data.strIngredient5 && `<p>ing 5:${this._data?.strIngredient5}</p>`}
                ${this._data.strIngredient6 && `<p>ing 6:${this._data?.strIngredient6}</p>`}
                ${this._data.strIngredient7 && `<p>ing 7:${this._data?.strIngredient7}</p>`}
                ${this._data.strIngredient8 && `<p>ing 8:${this._data?.strIngredient8}</p>`}
                ${this._data.strIngredient9 && `<p>ing 9:${this._data?.strIngredient9}</p>`}
                <div class="content">
                <p class="price-content">price: <span class="price">${(+this._data.idMeal / (Math.random() * 100000)).toFixed(2)}</span> $</p>
                <form class="add-form">
                <div class="input-group">
...
exports.default = new mealView();

},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1eom8":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class wishlistView extends (0, _viewDefault.default) {
    ...
    removeFromWishlistHandlerClick(handler, data) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".delete")) {
                let id;
                data.find((meal)=>{
                    if (e.target.closest(".wishlist-meal").dataset.id === meal.idMeal) id = meal.idMeal;
                });
                console.log(id);
                const mealID = e.target.closest(".wishlist-meal").dataset.id;
                document.querySelector("#app").querySelectorAll(".meal").forEach((meal)=>{
                ...
                if (document.querySelector(".wishlist-counter").textContent == 0) document.querySelector(".wishlist-meals").innerHTML = `<p>you have no wishlist item</p>`;
                handler(id);
            }
        });
    }
}
exports.default = new wishlistView();

},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"IaDwG":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
  ...
  `);
    ...
    _generateMarkup() {
        ...
        <p class="name">name : <span class="item-name">${meal.name}</span></p>
          <p class="price">total price : <span class="total-price">${meal.price} X ${meal.amount} = ${meal.price * meal.amount}</span></p>
...
},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"hPmPF":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class accountView extends (0, _viewDefault.default) {
    _parentElement = document.querySelector("#meals");
        ...
        if (document.querySelector(".registerBtn")) {
            ...
            email.addEventListener("focusout", (e)=>{
                if (!e.target.value.match(validRegex)) {
                    ...
                    document.querySelector(".emailMessage").innerHTML = "inserted email is not valid";
                } else emailValue = e.target.value;
            });
            // phone Validation
            phone.addEventListener("focusout", (e)=>{
                if (!e.target.value.match(validPhoneNumber)) {
                    e.target.style.border = "2px solid rgb(248, 108, 108)";
                    document.querySelector(".phoneMessage").innerHTML = "inserted phone number is not valid";
                } else phoneValue = e.target.value;
            });
            password.addEventListener("focusout", (e)=>{
                if (!e.target.value.match(validPassword)) {
                    e.target.style.border = "2px solid rgb(248, 108, 108)";
                    document.querySelector(".passwordMessage").innerHTML = "inserted password is not valid";
                } else passwordValue = e.target.value;
            });
            this._parentElement.addEventListener("click", (e)=>{
                console.log();
                if (e.target.closest(".registerBtn")) {
                    ...
                    if (!firstName) {
                        ...
                        document.querySelector("#first-name").style.border = "2px solid rgb(248, 108, 108)";
                    }
                    if (!lastName) {
                        document.querySelector("#last-name").focus();
                        document.querySelector("#last-name").style.border = "2px solid rgb(248, 108, 108)";
                    }
                    if (passwordValue !== repass) {
                        document.querySelector("#repass").focus();
                        document.querySelector("#repass").style.border = "2px solid rgb(248, 108, 108)";
                        document.querySelector(".repassMessage").innerHTML = "\u0631\u0645\u0632 \u0648\u0627\u0631\u062F \u0634\u062F\u0647 \u06CC\u06A9\u0633\u0627\u0646 \u0646\u06CC\u0633\u062A";
                    }
                    if (firstName && lastName && emailValue && phoneValue && passwordValue && passwordValue === repass) {
                        const user = {
                            firstName: firstName,
                            lastName: lastName,
                            email: emailValue,
                        ...
                        document.querySelector(".account").textContent = `${user.firstName}`;
                        render(user);
                        manageData(user);
                    }
                }
    ...
    loginBtnHandlerClick(handler) {
        this._parentElement.addEventListener("click", (e)=>{
            if (e.target.closest(".loginBtn")) {
                ...
                const activeUser = this._data.users.find((user)=>user.phone == emailORphone || user.email === emailORphone);
                if (activeUser && activeUser.password == pass) {
                    document.querySelector(".account").textContent = `${activeUser.firstName}`;
                    handler(activeUser);
                } else document.querySelector(".message").innerHTML = "user name or pass is wrong";
            }
        });
    }
    _generateMarkup() {
        ...
        if (!this._data.isLoggedIn) markup = `
        <section id="login-section">
...
},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["fA0o9","aenu9"], "aenu9", "parcelRequire1dd7")
